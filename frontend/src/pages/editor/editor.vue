<template>
        <el-container id="BOX_BASE"> <!--最高级容器-->
                <el-header> <!--放置-->
                    <el-menu
                    menu-trigger="click"
                    mode="horizontal"
                    ...
                    -->
                        ...
                        <!--帮助栏目-->
                        <el-menu-item><el-link href="README.md">帮助</el-link></el-menu-item>

                    </el-menu>
                </el-header>
                <el-container>
                    <el-aside id="BOX_PROJECT_FILE_LIST"> <!--当前项目文件栏-->

                        <el-tree :data="treeData" :props="defaultProps"/> <!--展示项目文件夹结构的树形控件-->
                    
                        
                    </el-aside>

                    <el-main id="BOX_FILE_EDITOR"> <!--编辑器空间-->
                        <el-tabs 
                        v-model="active_name" 
                        type="border-card"
                        editable
...
export default {
        ...
        methods: {
            handleMenuSelect() { //应对菜单选择

            },
            /*handleClick(tab, event) {
                //console.log(tab, event);
            },*/
            handleTabsEdit(targetName, action) { //应对动态添加/删除tabs，action: 'remove' | 'add'
                if (action === 'add')
                {
                    console.log('add!')
                }
...
<style>
    ...
    #BOX_PROJECT_FILE_LIST { /* 目前项目文件列表 */
        width: 200px;
        padding: 20px;
